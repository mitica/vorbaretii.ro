...
for (const pose of POSES) {
  for (const phase of [0, 0.3]) {
    test(`${pose} @${phase}: SVG valid, doar atribute, toate părțile (ADR-017)`, async () => {
      const svg = mascotSvg(pose, phase);
...
test("liniște @0: pixelii geometriei (corp, burtă, petec, creastă, ochi, cioc, picior)", async () => {
  const px = await render("liniste", 0);
...
test("bucurie @0: ochii fericiți în locul pupilei", async () => {
  const px = await render("bucurie", 0);
...
test("vorbește: gura se vede doar când ciocul e open (0 < phase < 0.5); phase 0 = rest", async () => {
  const rest = await render("vorbeste", 0);
  const open = await render("vorbeste", 0.3);
  const closed = await render("vorbeste", 0.7);
  assert.equal(
    rest({ x: 120, y: 113 }),
    "#feab2b",
    "ADR-017 — la phase 0 ciocul e închis (ipostaza de rest sub prefers-reduced-motion)"
  );
  assert.equal(open({ x: 120, y: 113 }), "#e0405a", "ADR-017 — gura la phase 0.3");
  assert.notEqual(closed({ x: 120, y: 113 }), "#e0405a", "ADR-017 — ciocul închis la phase 0.7");
});

test("gândește @0: privirea sus, aripa dreaptă ieșită în lateral", async () => {
  const calm = await render("liniste", 0);
  const thinking = await render("gandeste", 0);
  assert.equal(calm({ x: 97, y: 103 }), "#121315", "ADR-017 — pupila jos la liniște");
  assert.equal(thinking({ x: 97, y: 103 }), "#ffffff", "ADR-017 — pupila a urcat la gândește");
  assert.equal(thinking({ x: 97, y: 82 }), "#121315", "ADR-017 — pupila sus la gândește");
  assert.equal(calm({ x: 200, y: 140 }), "#353a85", "ADR-017 — aripa la corp la liniște");
  assert.equal(
    thinking({ x: 200, y: 140 }),
    "#3e93e9",
    "ADR-017 — petecul aripii ieșite la gândește"
  );
});

test("componenta Mascot: wrapper decorativ cu data-pose, mărime și SVG-ul inline", () => {
  const html = renderToStaticMarkup(<Mascot pose="salut" size={64} />);
  assert.ok(html.startsWith("<span"), "ADR-017 — wrapper span");
  assert.ok(html.includes('aria-hidden="true"'), "ADR-017 — decorativă");
  assert.ok(html.includes('data-pose="salut"'), "ADR-017 — starea pe wrapper armează animațiile");
