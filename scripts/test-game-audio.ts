/**
 * Legea vocii jocurilor (ADR-050, succesoarea ADR-050): fiecare joc cu voce
 * poartă audio-ul textelor ei curente, într-o singură key de voce; orfanii nu
 * se servesc; fiecare fișier ține bugetul. NOU (ADR-050): rostirea fără
...
test("ADR-050: set exact → verde; dir absent → verde (opt-in per joc)", () => {
  const ok = checkVoice({
    slug: "ghicitori",
    expected: [A, B],
    key: KEY,
...
test("ADR-050: orphan → roșu; rostirea fără fișier e legală, pentru orice joc (element mut)", () => {
  ...
  assert.match(orphan[0]!, /ADR-050/);
  assert.match(orphan[0]!, /orphan/);
  assert.match(orphan[0]!, /voce-jocuri ghicitori/);
  // ADR-050: un joc „normal" (nu doar curiozitățile) tolerează lipsa —
...
test("ADR-050: a doua key pe disc → roșu; fișier peste buget → roșu", () => {
  ...
  assert.match(heavy[0]!, /ADR-050/);
  assert.match(heavy[0]!, /buget/);
});

test("ADR-050: rostirile fiecărui joc cu voce sunt nevide, fără dubluri, fără text gol", () => {
  for (const slug of Object.keys(VOICED_GAMES)) {
    ...
    assert.ok(utterances.length > 0, `ADR-050 — ${slug}: niciun text de rostit`);
    assert.equal(
      new Set(utterances).size,
      utterances.length,
      `ADR-050 — ${slug}: utterances duplicate`
    );
    for (const r of utterances)
      assert.ok(r.trim().length > 0, `ADR-050 — ${slug}: utterance goală`);
    const hashes = utterances.map(hashId);
    assert.equal(
      new Set(hashes).size,
      hashes.length,
      `ADR-050 — ${slug}: două rostiri cu același hash`
    );
  }
});

test("ADR-050: URL-ul servit și directorul de pe disc sunt același loc", () => {
  const url = audioPath("ghicitori", "Oul");
  assert.ok(
    url.startsWith("/assets/audio/jocuri/ghicitori/"),
    "ADR-050 — URL-ul nu e sub /assets/audio/jocuri"
  );
  assert.ok(
    VOICE_DIR.endsWith("public/assets/audio/jocuri"),
    "ADR-050 — directorul nu e sub public/assets/audio/jocuri"
  );
});

test("ADR-050: citirea discului vede cheile și fișierele cheii cerute", () => {
...
test("ADR-050: discul real — niciun joc cu voce n-are orfan sau key veche", () => {
...
test("ADR-050: availableUtterances — fișierul de pe disc intră în mulțime, cel lipsă nu", () => {
  ...
  try {
    ...
    assert.equal(available.has(B), false, "ADR-050 — rostirea fără fișier nu intră în mulțime");
  } finally {
    process.chdir(before);
    rmSync(root, { recursive: true, force: true });
  }
...
test("ADR-050: availableUtterances — joc fără director → mulțime goală", () => {
  const root = mkdtempSync(join(tmpdir(), "voce-disponibile-"));
  const before = process.cwd();
  process.chdir(root);
  try {
...
// al legii (ADR-050): se probă prin execuție directă, ca `checkVoice` mai
// sus; cablarea lui până la buton se probă mai jos, prin sursă.
test("ADR-050: canSpeakFor — legea butonului, probată prin execuție", () => {
  const utterance = "Oul";
  const hash = hashId(utterance);
  assert.equal(
    canSpeakFor(new Set([hash]), utterance),
    true,
    "ADR-050 — hash prezent în mulțime, butonul ar trebui să poată vorbi"
  );
  assert.equal(
    canSpeakFor(new Set(["altceva"]), utterance),
    false,
    "ADR-050 — hash absent din mulțime, butonul nu poate vorbi"
  );
  assert.equal(
    canSpeakFor(new Set(), utterance),
    false,
    "ADR-050 — mulțime goală, butonul nu poate vorbi"
  );
  assert.equal(
    canSpeakFor(new Set([hash]), null),
    false,
    "ADR-050 — fără rostire curentă, butonul nu poate vorbi"
  );
});
...
test("ADR-050: nimic la încărcare — elementul audio se creează la prima atingere, fără preload/autoPlay", () => {
  ...
  for (const source of [context, button]) {
    assert.ok(!/autoPlay|preload/.test(source), "ADR-050 — preload/autoPlay în componentele vocii");
  }
  assert.equal(
    context.match(/new Audio\(/g)?.length,
    1,
    "ADR-050 — exact un `new Audio(`, în deblocare"
  );
  const unlock = context.slice(
    context.indexOf("const unlock"),
    context.indexOf("useEffect(() => {\n    window.addEventListener")
  );
  assert.ok(unlock.includes("new Audio("), "ADR-050 — `new Audio(` trăiește în `unlock`");
  assert.ok(
    context.includes('"pointerdown", unlock, { once: true'),
    "ADR-050 — deblocarea e legată de prima atingere"
  );
  assert.ok(
    context.includes("loadJson<unknown>(SETTING_KEY, true)"),
    "ADR-050 — vocea e pornită implicit, setarea în memoria locală"
  );
  assert.ok(
    context.includes('typeof stored === "boolean"'),
    "ADR-050 — setarea citită din memoria locală se coerce, nu se crede pe cuvânt"
  );
});

test("ADR-050: butonul e cablat la legea testată — canSpeak vine din canSpeakFor, nu dintr-o condiție paralelă", () => {
  const context = readFileSync(join(VOICE, "context.tsx"), "utf8");
  const button = readFileSync(join(VOICE, "mascot-voice.tsx"), "utf8");
  assert.ok(
    button.includes("disabled={!voice.canSpeak}"),
    "ADR-050 — butonul nu se dezactivează pe canSpeak"
  );
  ...
  assert.ok(readyIdx >= 0, "ADR-050 — `ready` nu vine din canSpeakFor(available, utterance)");
  assert.ok(canSpeakIdx >= 0, "ADR-050 — `canSpeak: url !== null` nu s-a găsit în sursă");
  assert.ok(readyIdx < canSpeakIdx, "ADR-050 — canSpeak nu e cablat DUPĂ canSpeakFor, în sursă");
  const wiring = context.slice(readyIdx, canSpeakIdx);
  assert.ok(
    wiring.includes("const url = ready && utterance"),
    "ADR-050 — `url` nu depinde de `ready` (canSpeakFor)"
  );
});

test("ADR-050: cele 8 jocuri cu voce raportează rostirea; cele 5 fără voce nu ating vocea", () => {
  for (const name of VOICED) {
    ...
    assert.ok(
      ...
      `ADR-050 — ${name} nu raportează rostirea curentă`
    );
  }
  for (const name of VOICELESS) {
    ...
    assert.ok(
      ...
      `ADR-050 — ${name} n-are voce în design, dar atinge vocea`
    );
  }
});
...
test("ADR-050: discul real — fiecare rostire comisa tine nivelul, capetele si formatul", async () => {
  // Corpus gol = verde vacuu (N6): jocul fara director trece, ca la ADR-050.
