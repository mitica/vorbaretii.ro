...
type ChildType = "boy" | "girl";

type Props = {
  child?: ChildType;
};

const getImages = (child: ChildType) => {
  const size = 256;
  const boy = [
...
export default function BoringToHappy({ child = "boy" }: Props) {
  ...
  const images = getImages(child).map((image, i) => {
    // first image in the center, 1/3 of min height/wight:
    if (i === 0) {
      return (
        <img
          key={i}
          className={classNames(imageClassName, "grayscale h-1/4")}
    ...
    return (
      <img
        key={i}
        className={classNames(imageClassName, "h-2/6")}
        src={`/assets/images/${image}`}
        alt={image}
