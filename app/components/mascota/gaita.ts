 ...
 * `gaitaSvg(stare, faza)` randează o ipostază ca SVG DOAR cu atribute de
 ...
 * animațiile Tailwind (`motion-safe:group-data-[stare=…]:animate-…`); `faza`
 * (0..1) mișcă atributele pentru consumatorii fără CSS.
 */

export const STARI = ["liniste", "salut", "vorbeste", "bucurie", "gandeste"] as const;
...
const UMAR = "66 112";
const OGLINDA = "matrix(-1 0 0 1 239.5 0)";
const UNGHI_SUS = 103;

const ARIPA =
  "M64 112 C 46 117, 31 140, 35 160 C 37 171, 43 177, 49 174 C 59 168, 68 142, 64 112 Z";
const PETEC =
...
const ANIM = {
  tot: "motion-safe:group-data-[stare=liniste]:animate-legana motion-safe:group-data-[stare=vorbeste]:animate-cap-vorbeste motion-safe:group-data-[stare=bucurie]:animate-sare",
  pleoape: "motion-safe:animate-clipit",
  aripaStSus:
    "origin-[66px_112px] motion-safe:group-data-[stare=salut]:animate-flutura motion-safe:group-data-[stare=bucurie]:animate-aripi-sus-st",
  aripaDrSus: "origin-[0px_0px] motion-safe:group-data-[stare=bucurie]:animate-aripi-sus-dr",
  ciocJos: "origin-[120px_112px] motion-safe:group-data-[stare=vorbeste]:animate-vorbeste",
} as const;

const op = (vizibil: boolean) => (vizibil ? "1" : "0");
const path = (d: string, attrs: string) => `<path d="${d}" ${attrs}/>`;
const grup = (attrs: string, inner: string) => `<g ${attrs}>${inner}</g>`;

/** Aripile: un desen (defs) plasat de patru ori — jos/sus × stânga/dreapta. */
function aripi(stare: Stare, faza: number): string {
  const ridicata = stare === "salut" || stare === "bucurie";
  const ambele = stare === "bucurie";
  const unghi = (UNGHI_SUS + 11 * Math.sin(2 * Math.PI * faza)).toFixed(1);
  const gandeste = stare === "gandeste" ? " rotate(24 66 112)" : "";
  const use = '<use href="#aripa" xlink:href="#aripa"/>';
  const def = grup(
    'id="aripa"',
    path(ARIPA, `fill="${CULORI.aripa}"`) +
      path(PETEC, `fill="${CULORI.petec}"`) +
      `<path d="M38 148 L 54 153" stroke="${CULORI.negru}" stroke-width="4.6" stroke-linecap="round"/>` +
      `<path d="M37 159 L 51 163" stroke="${CULORI.negru}" stroke-width="4.6" stroke-linecap="round"/>`
  );
  return (
    `<defs>${def}</defs>` +
    grup(`class="aripa-st-jos" opacity="${op(!ridicata)}"`, use) +
    grup(
      `class="aripa-st-sus ${ANIM.aripaStSus}" opacity="${op(ridicata)}" transform="rotate(${unghi} ${UMAR})"`,
      use
    ) +
    grup(`class="aripa-dr-jos" opacity="${op(!ambele)}" transform="${OGLINDA}${gandeste}"`, use) +
    grup(
      `class="aripa-dr-sus ${ANIM.aripaDrSus}" opacity="${op(ambele)}" transform="${OGLINDA} rotate(${unghi} ${UMAR})"`,
      use
    )
  );
}
...
function picioare(): string {
  const labe = path(LABA_ST, "") + path(LABA_ST, `transform="${OGLINDA}"`);
  return grup(
    `fill="${CULORI.laba}" stroke="${CULORI.labaMargine}" stroke-width="1.2" stroke-linejoin="round"`,
...
function sprancene(stare: Stare): string {
  const grupT = stare === "bucurie" ? ' transform="translate(0 -5)"' : "";
  const stangaT = stare === "gandeste" ? ' transform="translate(0 -6)"' : "";
  return grup(
    `class="sprancene" fill="${CULORI.negru}"${grupT}`,
    `<path class="spranceana-st" d="${SPRANCEANA_ST}"${stangaT}/>` +
      `<path class="spranceana-dr" d="${SPRANCEANA_DR}"/>`
  );
}

/** Un ochi deschis: alb, pupilă, lucire — pupila și lucirea privesc în sus la gândește. */
function ochi(cx: number, priveste: string): string {
  const px = cx + 2.2;
  const lx = cx - 1.6;
  return (
    `<circle cx="${cx}" cy="93.8" r="14.3" fill="${CULORI.alb}"/>` +
    `<circle class="pupila" cx="${px}" cy="94.5" r="10.6" fill="${CULORI.pupila}"${priveste}/>` +
    `<circle class="lucire" cx="${lx}" cy="91" r="3.3" fill="${CULORI.alb}"${priveste}/>`
  );
}

/** Ochii: deschiși (cu pleoapele care clipesc) sau fericiți — ambele grupuri, comutate prin opacitate. */
function ochii(stare: Stare, faza: number): string {
  const fericiti = stare === "bucurie";
  const clipeste = stare === "liniste" && faza >= 0.92 && faza < 0.96;
  const priveste = stare === "gandeste" ? ' transform="translate(-4 -5)"' : "";
  const pleoape = grup(
    `class="pleoape ${ANIM.pleoape}" fill="${CULORI.corp}" opacity="${op(clipeste)}"`,
    '<circle cx="94.8" cy="93.8" r="15.3"/><circle cx="144.7" cy="93.8" r="15.3"/>'
  );
  const deschisi = grup(
    `class="ochi-deschisi" opacity="${op(!fericiti)}"`,
    ochi(94.8, priveste) + ochi(144.7, priveste) + pleoape
  );
  const arcuri = grup(
    `class="ochi-fericiti" opacity="${op(fericiti)}" stroke="${CULORI.pupila}" stroke-width="4.5" stroke-linecap="round" fill="none"`,
    '<path d="M82 97 Q94.8 84 107.5 97"/><path d="M132 97 Q144.7 84 157.5 97"/>'
  );
  return deschisi + arcuri;
}
...
function cioc(stare: Stare, faza: number): string {
  ...
  return (
    ...
    path(MUSTATA_DR, `fill="${CULORI.negru}"`) +
    grup(
      'transform="translate(120 108) scale(1.1) translate(-120 -108)"',
      `<path d="M113 111.2 L127 111.2 L120 116.5 Z" fill="${CULORI.gura}"/>` +
        path(CIOC_SUS, `fill="${CULORI.cioc}"`) +
        grup(
          `class="cioc-jos ${ANIM.ciocJos}"${mandibula}`,
          path(CIOC_JOS, `fill="${CULORI.ciocJos}"`)
        )
    )
  );
}
...
function miscareTot(stare: Stare, faza: number): string {
  if (stare === "bucurie") return `translate(0 ${(-16 * Math.sin(Math.PI * faza)).toFixed(1)})`;
  if (stare === "vorbeste") return `translate(0 ${(-2 * Math.sin(2 * Math.PI * faza)).toFixed(1)})`;
  if (stare === "liniste") return `translate(0 ${(-3 * Math.sin(2 * Math.PI * faza)).toFixed(1)})`;
  return "translate(0 0)";
}

/** Ipostaza `stare` la faza `faza` (0..1), ca SVG doar cu atribute de prezentare. */
export function gaitaSvg(stare: Stare, faza = 0): string {
  const tot = grup(
    `class="tot ${ANIM.tot}" transform="${miscareTot(stare, faza)}"`,
    aripi(stare, faza) +
      picioare() +
      corp() +
      sprancene(stare) +
  ...
  return (
    ...
    `viewBox="0 0 240 240" aria-hidden="true" focusable="false">${tot}</svg>`
  );
}
